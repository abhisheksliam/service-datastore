...
exports.jsonToDistJava = function (scriptData) {
    ...
    if (scriptData.task_json[0] !== undefined) {
        ...
        var preJ = 'package testcase.' + taskData.appName + ';    import org.testng.annotations.Test;    import runner.TestRunner;    public class Test_' + taskData.id.replace(/\./gi, "_").trim() + '_' + taskData.scenario.toUpperCase().trim() + ' extends TestRunner {    ';
        ...
        if (pathwayListData !== undefined) {
            for (var q = 0; q < pathwayListData.length; q += 2) {
                ...
                if (arrayItem.constructor === Array) {
                    ...
                    var preJin = '\n    ' + '@Test (groups = {' + pathwayListData[q + 1] + '})' + 'public void ' + taskData.id.replace(/\./gi, "_").trim() + '_' + taskData.scenario.toUpperCase().trim() + '_' + (++testCount).toString() + '() throws Exception {            System.out.println("START..");            ';

                    var postJ = 'Thread.sleep(3000);            ' + 'System.out.println("DONE.");        }   \n';

                    arrayItem.forEach(function (arrayItem2) {
                        runJ = runJ + 'executeItem(' + '"' + taskData.id.trim() + '.' + taskData.scenario.trim() + '", ' + '"' + taskData.scenario.trim() + '", ' + '' + arrayItem2 + ');';
                    });
